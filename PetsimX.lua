--[[
-- THIS SCRIPT HAS BEEN CODED BY MTZ (https://discord.gg/49TpnW9y)
...
-- Important Variables
local SCRIPT_NAME = "MTZ PSX GUI"
local SCRIPT_VERSION = "v0.4" -- Hey MTZ remember to change it before updating lmao
...
if game.PlaceId == 6284583030 or game.PlaceId == 10321372166 or game.PlaceId == 7722306047 or game.PlaceId == 12610002282 then
	...
	if isfile("UI/ArrayField.lua") then
		...
		Rayfield = loadstring(game:HttpGet("https://raw.githubusercontent.com/MTZoffical/arraylift/main/v1"))()
	end	
	assert(Rayfield, "Oopps! Rayfield has not been loaded. Maybe try re-joining?") 
	

	local Window = Rayfield:CreateWindow({
	   Name = "Pet Simulator X | By MTZOFFİCAL ",
	   LoadingTitle = SCRIPT_NAME .. " " .. SCRIPT_VERSION,
	   LoadingSubtitle = "MTZoffical",
	   ConfigurationSaving = {
		  Enabled = true,
		  FolderName = "MTZ",
	...
	coroutine.wrap(function() 
		wait(0.5)
		if not isfile("MTZ/AcceptedTerms.txt") then 
			Window:Prompt({
				Title = 'Disclaimer',
				...
				Actions = {
					...
					Accept = {
						Name = "Ok",
						Callback = function()
							if not isfolder("MTZ") then makefolder("MTZ") end
							writefile("MTZ/AcceptedTerms.txt", "true")
						end,
	...
	local stats = autoFarmTab:CreateParagraph({Title = "Hello, <b><font color=\"#2B699F\">" .. LocalPlayer.DisplayName .. "</font></b>!", Content = "Thanks for using my script! - MTZ\nMake sure to join us at <b><font color=\"#2B699F\">https://discord.gg/49TpnW9y</font></b>"})
